...
void ASCCharacterBase::Tick(float DeltaTime)
{
	...
	AStretchyCatPlayerController* CastController = Cast< AStretchyCatPlayerController>(Controller);
	if (CastController != nullptr) {
		FHitResult OutHit(ForceInit);
		CastController->GetHitResultUnderCursor(ECC_Visibility, false, OutHit);
		if (OutHit.GetActor() != nullptr) {
			FRotator newLookAt = FRotationMatrix::MakeFromX(OutHit.ImpactPoint - GetActorLocation()).Rotator();
			newLookAt.Pitch = 0;
			newLookAt.Roll = 0;
			//FRotator NewRot = FMath::RInterpTo(GetActorRotation(), newLookAt, DeltaTime, 10);
